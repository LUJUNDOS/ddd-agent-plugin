...
| goal-executor | 调度传动：自驱循环（5 工位串联）/ 并行（worktree + spawn）/ 重试停滞 / active-goal 恢复 / verifier 子代理 |
...
## 16. v0.2.2 —— evolution-scan（长期区维护，FR-016/AC-10）

- **分工（无重复）**：memory-protocol 管"记 + 晋升"（短期沉淀，同类 ≥3 次或显批 → 长期区，**不改变**）；evolution-scan 管"长期区维护"——扫描**已晋升的长期区条目**，不做 ≥3 次提炼判断（避免与 memory-protocol 重复）。
- **流程**（`templates/evolution-scan.md`）：
  1. 扫描长期区条目（CLAUDE.md 固定条目等）
  2. 退休：30 天未触发 → 退休候选 → **用户确认后**清理/归档（Oracle 不可跳过）
  3. 合并/去重：同 root_cause 多条 → 合并保留最完整 + 累计 hits
  4. 冲突检测：新旧规则矛盾 → 交用户裁决
  5. 跨项目提炼：不同项目同类 error → 高层通用原则（applies_to 标注）
