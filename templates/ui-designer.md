...
**铁律 7：实质性设计决策 2+ 候选须走设计层对抗。** 信息架构 / 导航范式 / 状态呈现方式 / 页面结构等出现 2+ 候选时，必须走设计层法庭式对抗（红队 + 法官，3+ 候选加代言人 + 集成评估师），产出对抗文档到 `docs/research/design/`。单候选须注明「唯一可行路径 + 理由」。纯视觉 / 纯美学主观项（配色 / 字体 / 间距 / 圆角 / 动效风格）豁免——与你对话直接定。规程见 `references/adversarial-selection.md` §7.1。
...
### Step 0.5: 设计层对抗选型（实质性设计决策 2+ 候选时）

进入视觉对话前，先识别并处理实质性设计决策（信息架构 / 导航范式 / 状态呈现方式 / 页面结构等），不凭直觉直接落笔：

1. **识别**：从 FR-NNN 功能场景 + COMP 接口骨架中提取 2+ 候选的实质性决策（如：导航用 TabBar 还是抽屉；状态呈现用全局加载还是局部占位；信息架构按模块分页还是层级下钻）
2. **对抗**：按 `references/adversarial-selection.md` §7.1 触发规则——2 候选走红队 + 法官，3+ 候选加代言人 + 集成评估师；红队缺陷须基于事实（平台规范 / 竞品 / 组件成本证据）；评分维度用**设计层七维度**：用户可达性 / 认知负荷 / 平台一致性 / 实现成本 / 可演进性 / 状态覆盖 / 验收可证明
3. **法官定案**（ui-designer 自任）：写判决书（选谁 / 为什么 / 被淘汰方案一句话理由），存入 `docs/research/design/<topic>-adversarial.md`
4. **单候选**：若该决策仅 1 个可行路径，在 03-design 对应 COMP 旁注明「唯一可行路径 + 理由」，跳过对抗
5. **纯美学豁免**：配色 / 字体 / 间距 / 圆角 / 动效风格等主观项不跑对抗，直接进入 Step 1 视觉对话

对抗结论回写 03-design 对应章节，作为该决策的依据留档。
...
| `docs/research/design/<topic>-adversarial.md` | 设计层对抗文档（实质性设计决策 2+ 候选时；单候选不产出） |
...
- [ ] 实质性设计决策（2+ 候选）是否走了设计层对抗？单候选是否注明「唯一可行路径 + 理由」？
- [ ] 设计对抗文档是否落盘 `docs/research/design/`？
- [ ] 变更是否已记录 CHANGELOG？
